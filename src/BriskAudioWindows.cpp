...
namespace BriskAudio {
	...
	NativeDeviceHandle::NativeDeviceHandle()
	{
		...
		pOnAnyDeviceAdd = nullptr;
		pOnAnyDeviceRemove = nullptr;
		pOnAnyDeviceStateChange = nullptr;
		referenceCount_ = 1;
	}
	...
	HRESULT STDMETHODCALLTYPE NativeDeviceHandle::OnDefaultDeviceChanged(EDataFlow flow, ERole role, LPCWSTR pwstrDeviceId)
	{
		...
		// This function is called for each role change, so invoke the callback only once
		if (pOnDefaultDeviceChange != nullptr) {
	...
	{
		if (pOnAnyDeviceStateChange == nullptr) {
			return S_FALSE;
		}
		...
		case DEVICE_STATE_ACTIVE:
			pOnAnyDeviceStateChange(deviceName, "ACTIVATED");
			break;
		case DEVICE_STATE_DISABLED:
			pOnAnyDeviceStateChange(deviceName, "DISABLED");
			break;
		case DEVICE_STATE_NOTPRESENT:
			pOnAnyDeviceStateChange(deviceName, "ABSENT");
			break;
		case DEVICE_STATE_UNPLUGGED:
			pOnAnyDeviceStateChange(deviceName, "UNPLUGGED");
			break;
		}
	...
	HRESULT STDMETHODCALLTYPE NativeDeviceHandle::OnNotify(PAUDIO_VOLUME_NOTIFICATION_DATA pNotify)
	{
		...
		if (FAILED(pDevice->GetId(&deviceId))) {
			return S_FALSE;
		}

		if (FAILED(spEnumerator->GetDevice(deviceId, &pTempDevice))) {
			return S_FALSE;
		}
		...
		if (pNotify->guidEventContext != eventContext) {
			if (pNotify->bMuted == TRUE) {
				...
				if (pOnVolumeChange != nullptr) {
					pOnVolumeChange(deviceName, pNotify->fMasterVolume);
				}
			}
		}
	...
	{
		EDataFlow flow = (aType == DeviceType::PLAYBACK) ? eRender : (aType == DeviceType::CAPTURE) ? eCapture : eAll;
	...
	Exit openDefaultDevice(Device& arDevice, DeviceType aType)
	{
		...
		if (FAILED(spEnumerator->GetDefaultAudioEndpoint(flow, eConsole, &arDevice.pDevice))) {
			return Exit::FAILURE;
		}
	...
	Exit openDevice(Device& arDevice, std::string aDeviceName)
	{
		...
		for (unsigned int i = 0; i < count; i++) {
			...
			if (aDeviceName == arDevice.name) {
				...
				arDevice.type = (flow == eRender) ? DeviceType::PLAYBACK : (flow == eCapture) ? DeviceType::CAPTURE : DeviceType::ALL;
